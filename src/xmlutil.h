...
#define CHILD(NODE,KEYWORD) getNodeByName (NODE, XMLPREFIX(KEYWORD))
#define CHILDSTRING(NODE,KEYWORD) getNodeContentOrComplain (CHILD(NODE,KEYWORD), XMLPREFIX(KEYWORD))
#define CHILDINT(NODE,KEYWORD) decToSignedInt((const char*) CHILDSTRING(NODE,KEYWORD))
#define CHILDFLOAT(NODE,KEYWORD) atof((const char*) CHILDSTRING(NODE,KEYWORD))
#define CHILDHEX(NODE,KEYWORD) hexToUnsignedInt ((const char*) CHILDSTRING(NODE,KEYWORD))
...
#define NODEINTVAL(NODE) decToSignedInt ((const char*) getNodeContent(NODE))
#define NODEFLOATVAL(NODE) atof ((const char*) getNodeContent(NODE))
#define NODEHEXVAL(NODE) hexToUnsignedInt ((const char*) getNodeContent(NODE))
