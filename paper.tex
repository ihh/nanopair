...
%\newpage
\section{Abstract}
State machine algorithms for aligning Nanopore read pairs.
...
Perform the following steps
\begin{itemize}
\item Prefilter read pairs using k-mer threshold heuristics
\item Perform Viterbi alignment using intersection of two rich models
\end{itemize}
...
\subsubsection{Prefiltering heuristic}

(Omit this step in first implementation?)

\begin{itemize}
\item Build database of k-mer frequencies across all reads
\item Compute likelihood of each read using a ``bag of words'' model
\item Compute likelihood of each read-pair using a ``bag of paired words'' model
\item Compute posterior probability of a match
\end{itemize}
